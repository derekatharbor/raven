...
export default function EditorTabs({ tabs, activeTabId, onTabSelect, onTabClose, onNewTab }: EditorTabsProps) {
  return (
    <div className="h-9 flex items-center bg-[#FBF9F7] border-b border-gray-200 overflow-x-auto">
      {/* Tabs */}
      {tabs.map(tab => (
        <div
          key={tab.id}
          ...
          className={`
            group flex items-center gap-2 h-full px-3 cursor-pointer transition-colors border-r border-gray-200 min-w-0 max-w-[180px] flex-shrink-0
      ...
      {/* New tab button - inline right after last tab */}
      <button
        onClick={onNewTab}
        className="h-full px-2.5 hover:bg-gray-100 cursor-pointer transition-colors flex-shrink-0"
        title="New document"
      >
